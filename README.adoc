...
* Import the demo realm `holrealm.json` (located inside the `scripts` folder of this repo) 

==== Secure the services

(Be sure to start from the `scripts` folder)
...
For both of these scripts, make sure they have the name `import.sql` and put them in `main/src/resources` for each of the projects, they will be run at startup. 
